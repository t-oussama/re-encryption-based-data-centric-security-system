...
class AontBasedEncryption(object):
    ...
    def encrypt(self, ctr, prfKey1, prfKey2, prfKey3, message):
        res = lib.AontBasedEncryption_Encrypt(self.obj, ctr, prfKey1, prfKey2, prfKey3, message, len(message), len(message)//L)
        return bytes(res[0][0:L]), bytes(res[1][0:len(message)+L])

    def decrypt(self, ctr, prfKey1, prfKey2, prfKey3, cipher, iv):
        res = lib.AontBasedEncryption_Decrypt(self.obj, ctr, prfKey1, prfKey2, prfKey3, cipher, len(cipher), iv, len(cipher)//L - 1)
    ...
    def re_encrypt(self, reEncryptionKey1, originalKey2, newKey2, reEncryptionKey3, iv, cipher):
        ...
        res = lib.AontBasedEncryption_ReEncrypt(self.obj, p_reEncryptionKey1, p_originalKey2, p_newKey2, p_reEncryptionKey3, iv, cipher, len(cipher)//L - 1)
        return bytes(res[0][0:L]), bytes(res[1][0:len(cipher)])
